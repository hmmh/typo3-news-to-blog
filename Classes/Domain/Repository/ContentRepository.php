...
class ContentRepository extends AbstractRepository
{
     ...
     * @return mixed
     */
    public function getImportedContentElement(int $newsId, int $pageId, int $ceId = 0, int $sysLanguageUid = 0)
    {
        ...
        return $queryBuilder->select('*')
            ...
            ->where(
                ...
                $queryBuilder->expr()->eq('tx_newstoblog_ce', $queryBuilder->createNamedParameter($ceId, \PDO::PARAM_INT))
            )
            ->setMaxResults(1)
            ->execute()
            ->fetch();
     ...
     * @return mixed
     */
    public function getContentFromPage(int $pageId)
    {
        $queryBuilder = $this->getQueryBuilder('tt_content');
        $this->setDeletedRestrictionOnly($queryBuilder);
